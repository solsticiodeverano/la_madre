...
function preloadEarthAssets(){
  ...
  a.geo = loadJSON('data/custom.geo',
    (g)=>{ a.geo = g; },
  ...
  const soundFiles = {
    africa:'africa.mp3', africa2:'africa2.mp3', america:'america.mp3',
    arab:'arab.mp3', arab2:'arab2.mp3', asia:'asia.mp3', asia2:'asia2.mp3',
    ocean:'ocean.mp3', ocean2:'ocean2.mp3', polos:'polos.mp3',
...
class InstrumentEarth{
  ...
  drawHUD(intro){
    ...
    fill(0,160*fade); noStroke(); rectMode(CORNER);
    rect(18, height-132, 410, 112, 16);
    fill(255,225*fade); textAlign(LEFT, TOP); textSize(15);
    text(`LAT  : ${nf(this.selectedLat,1,2)}\nLON  : ${nf(this.selectedLon,1,2)}\nPAIS : ${this.selectedCountry}\nZONA : ${this.zone}\nAUDIO: ${this.audioReady ? 'activo' : 'click para activar'}`, 36, height-118);

    fill(0,150*fade); rect(width*.09, 30, width*.82, 92, 20);
    textAlign(CENTER, CENTER); textSize(22); fill(255,220,180,230*fade);
    text(this.phrase || 'El archivo todavía no habla desde este lugar.', width*.11, 36, width*.78, 82);
    hint(ENABLE_DEPTH_TEST);
  }

  drawPadUI(){
    resetMatrix(); camera(); noLights(); hint(DISABLE_DEPTH_TEST);
    const s=18, gap=8, x=width-135, y=height-42;
    fill(255,150); textAlign(RIGHT,CENTER); textSize(11); text(`${this.mode} ${floor(this.beat/4)}`, x-8, y+s/2);
